...
public class HashAndChainedListSearch implements LzssSearchMethod {
	 ...
	 */
	public HashAndChainedListSearch(final int DictionarySize, final int MaxMatch, final int Threshold,
			final byte[] TextBuffer) {
	 ...
	 */
	public HashAndChainedListSearch(final int DictionarySize, final int MaxMatch, final int Threshold,
			final byte[] TextBuffer, final int SearchLimitCount) {
		this(DictionarySize, MaxMatch, Threshold, TextBuffer, HashDefault.class.getName(),
				SearchLimitCount);
	}
	 ...
	 */
	public HashAndChainedListSearch(final int DictionarySize, final int MaxMatch, final int Threshold,
			final byte[] TextBuffer, final String HashMethodClassName) {
	 ...
	 */
	public HashAndChainedListSearch(final int DictionarySize, final int MaxMatch, final int Threshold,
			final byte[] TextBuffer, final String HashMethodClassName, final int SearchLimitCount) {
		if (0 < SearchLimitCount) {
			...
			try {
				this.hashMethod = (HashMethod) Factory.createInstance(HashMethodClassName,
	...
	public void put(final int position) {
		final int hash = this.hashMethod.hash(position);
	...
	public int searchAndPut(final int position) {
		...
		final int scanlimit = Math.max(this.DictionaryLimit, position - this.DictionarySize);
		//------------------------------------------------------------------
		//  連結リストの長さが長すぎる場合 offset を使用して
		//  連結リストの短いハッシュ値を使う。
		final int poshash = this.hashMethod.hash(position);
		int offhash = poshash;
		int offset = 0;
		...
		while (true) {
			...
			if (0 < offset && matchlen < this.hashMethod.hashRequires() + offset) {
				offset = 0;
	...
	public void slide() {
		...
		for (int i = 0; i < this.hashTable.length; i++) {
			final int pos = this.hashTable[i] - this.DictionarySize;
	...
	public int searchAndPut(final int position, final int[] matchposs) {
		int matchlen = this.Threshold - 1;
		int matchpos = position;
		final int maxmatch = this.MaxMatch;
		final int scanlimit = Math.max(this.DictionaryLimit, position - this.DictionarySize);
		int searchCount = this.SearchLimitCount;
		...
		while (scanlimit < scanpos && 0 < searchCount--) {
			if (this.TextBuffer[scanpos + matchlen] == this.TextBuffer[position + matchlen]) {
				...
				if (matchlen < len) {
					...
					final int end = Math.min(len + 1 - this.Threshold, matchposs.length);
					while (i < end)
						matchposs[i++] = scanpos;
					matchpos = scanpos;
					matchlen = len;
