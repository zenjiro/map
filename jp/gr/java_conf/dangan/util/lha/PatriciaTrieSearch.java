...
public class PatriciaTrieSearch implements LzssSearchMethod {
	...
	public PatriciaTrieSearch(final int DictionarySize, final int MaxMatch, final int Threshold, final byte[] TextBuffer) {
		...
		this.hashTable = new int[PatriciaTrieSearch.generateProbablePrime(this.DictionarySize
				+ (this.DictionarySize >> 2))];
	...
	public void put(final int position) {
		...
		final int posnode = (position & (this.DictionarySize - 1)) + this.DictionarySize;
		...
		while (true) {
			...
			while (matchlen < max
					&& (this.TextBuffer[matchpos + matchlen] == this.TextBuffer[position + matchlen])) {
				matchlen++;
			}
	...
	public int searchAndPut(final int position) {
		...
		if (scannode != PatriciaTrieSearch.UNUSED) {
			while (true) {
				...
				while (matchlen < max
						&& (this.TextBuffer[matchpos + matchlen] == this.TextBuffer[position
								+ matchlen])) {
					matchlen++;
				}
				if (matchlen == max && matchlen < this.MaxMatch) {
		...
		if (2 < this.TextBuffer.length - position) {
			...
			while (matchnode != PatriciaTrieSearch.UNUSED) {
				...
				if (matchnode < this.DictionarySize) {
					...
					scanpos = (lastPutPos < matchnode ? matchnode - this.DictionarySize : matchnode);
				}
	...
	public void slide() {
		...
		for (int i = 0; i < this.position.length; i++) {
			final int pos = this.position[i] - this.DictionarySize;
			if (0 < pos) {
				this.position[i] = pos;
			} else {
				this.position[i] = 0;
		...
		if (this.parent[node] != PatriciaTrieSearch.UNUSED) {
			...
			if (0 < parent) { //parent が PATRICIA Trie の根で無い場合 true となる条件式
				...
				if (this.childnum[parent] <= 1) {
					this.contractNode(this.child(parent, this.TextBuffer[this.position[parent]
							+ this.level[parent]] & 0xFF));
				}
			}
		}
	}
	...
	private void attachNode(final int parentnode, final int childnode, final int ch) {
		...
		final int node = this.hashTable[hash];
	...
	private void contractNode(final int node) {
		final int parentnode = this.parent[node];
