...
public class LhaOutputStream extends OutputStream {
	...
	public void putNextEntryNotYetCompressed(final LhaHeader header) throws IOException {
		...
		this.tempOut = CompressMethod.connectEncoder(this.temp.getOutputStream(), header
				.getCompressMethod(), this.property);
	}
	...
	public void closeEntry() throws IOException {
		if (this.header != null) {
			...
			if (this.temp.length() < this.length) {
				...
				final String method = this.header.getCompressMethod();
				this.header.setOriginalSize(this.length);
	...
	private static class TemporaryFile implements Temporary {
		...
		private class TemporaryFileInputStream extends InputStream {
			...
			public int read() throws IOException {
				final long pos = TemporaryFile.this.tempfile.getFilePointer(); //throws IOException
