...
public class LhaProperty {
	...
	public static String getProperty(final String key) {
		final String def = LhaProperty.property.getProperty(key);
		try {
	 ...
	 */
	public static Properties getProperties() {
		final Properties property = (Properties) LhaProperty.property.clone();
	...
	private static Object parseConstructor(final String source, final Hashtable substitute, final String[] packages) {
		...
		final String arguments = source.substring(source.indexOf('(') + 1, source.lastIndexOf(')'))
				.trim();
		classname = LhaProperty.applyPackages(classname, packages);
		Object[] args;
		if (!arguments.equals("")) {
			final StringTokenizer tokenizer = new StringTokenizer(arguments, ",()[]", true);
			final Stack<Comparable> stack = new Stack<Comparable>();
			int pos = 0;
			while (tokenizer.hasMoreTokens()) {
	...
	private static final Properties createDefaultProperty() {
		...
		property
				.put("lha.lzs.encoder",
						"LzssOutputStream( PostLzsEncoder( out ), HashAndChainedListSearch, [ HashShort ] )");
		property.put("lha.lz4.encoder", "out");
		property.put("lha.lz5.encoder",
				"LzssOutputStream( PostLz5Encoder( out ), HashAndChainedListSearch )");
		property.put("lha.lhd.encoder", "out");
		property.put("lha.lh0.encoder", "out");
		property.put("lha.lh1.encoder",
				"LzssOutputStream( PostLh1Encoder( out ), HashAndChainedListSearch )");
		property.put("lha.lh2.encoder",
				"LzssOutputStream( PostLh2Encoder( out ), HashAndChainedListSearch )");
		property.put("lha.lh3.encoder",
				"LzssOutputStream( PostLh3Encoder( out ), HashAndChainedListSearch )");
		property.put("lha.lh4.encoder",
				"LzssOutputStream( PostLh5Encoder( out, -lh4- ), HashAndChainedListSearch )");
		property.put("lha.lh5.encoder",
				"LzssOutputStream( PostLh5Encoder( out, -lh5- ), HashAndChainedListSearch )");
		property.put("lha.lh6.encoder",
				"LzssOutputStream( PostLh5Encoder( out, -lh6- ), HashAndChainedListSearch )");
		property.put("lha.lh7.encoder",
				"LzssOutputStream( PostLh5Encoder( out, -lh7- ), HashAndChainedListSearch )");
		//------------------------------------------------------------------
		//  decoders
	...
	private static final boolean isJapanese(final String encoding) {
		...
		final String[] Aliases = { "eucjis", "euc-jp", "eucjp", "x-euc-jp", "x-eucjp", //Aliases of "EUC_JP"
