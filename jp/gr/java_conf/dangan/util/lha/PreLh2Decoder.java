...
public class PreLh2Decoder implements PreLzssDecoder {
	...
	public PreLh2Decoder(final InputStream in) {
		if (in != null) {
			...
			this.offHiHuffman = new DynamicHuffman(PreLh2Decoder.DictionarySize >> 6, 1);
			this.position = 0;
	...
	public int readCode() throws IOException {
		...
		while (0 <= node) {
			node = this.codeHuffman.childNode(node - (this.in.readBoolean() ? 1 : 0));//throws EOFException,IOException
		}
		int code = ~node;
	...
	public int readOffset() throws IOException {
		...
		while (0 <= node) {
			node = this.offHiHuffman.childNode(node - (this.in.readBoolean() ? 1 : 0));//throws EOFException,IOException
		}
