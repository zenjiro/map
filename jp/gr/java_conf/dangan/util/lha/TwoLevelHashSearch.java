...
public class TwoLevelHashSearch implements LzssSearchMethod {
	 ...
	 */
	public TwoLevelHashSearch(final int DictionarySize, final int MaxMatch, final int Threshold, final byte[] TextBuffer,
			final String HashMethodClassName) {
		...
		try {
			this.primaryHash = (HashMethod) Factory.createInstance(HashMethodClassName,
	...
	public void put(final int position) {
		final int phash = this.primaryHash.hash(position);
		final int base = this.primaryHashTable[phash];
		final int shash = this.secondaryHash(position, this.secondaryHashRequires[phash]);
	...
	public int searchAndPut(final int position) {
		...
		final int phash = this.primaryHash.hash(position);
		final int base = this.primaryHashTable[phash];
		final int requires = this.secondaryHashRequires[phash];
		final int shash = this.secondaryHash(position, requires);
		int scanpos = this.secondaryHashTable[base + shash];
		final byte[] buf = this.TextBuffer;
		...
		int revbits = 1;
		final int loopend = requires - Math.max(0, this.Threshold - this.primaryHash.hashRequires());
			...
			while (revbits < send) {
				...
				while (scanlimit <= scanpos) {
					if (buf[scanpos] == buf[position]) {
						...
						len = p - position;
						if (matchlen < len || (matchlen == len && matchpos < scanpos)) {
							matchpos = scanpos;
							matchlen = len;
							if (max <= p) {
								scanlimit = scanpos;
	...
	public int search(final int position, final int lastPutPos) {
		...
		final int phashRequires = this.primaryHash.hashRequires();
		if (phashRequires < this.TextBuffer.length - position) {
			...
			if (phashRequires + requires < this.TextBuffer.length - position) {
				...
				final int avail = this.TextBuffer.length - position - phashRequires;
	...
	public void slide() {
		...
		for (int i = 0; i < this.primaryHashTable.length; i++) {
			...
			final int bits = this.secondaryHashRequires[i] * 2;
			if (1 << (5 + bits) <= this.primaryCount[i]) {
				for (int j = 0; j < (1 << bits); j++) {
					this.divide(dummyIndex, secondaryIndex, this.primaryHash.hashRequires()
				...
				for (int j = 0; j < (1 << bits); j++) {
					final int pos = this.secondaryHashTable[secondaryIndex++] - this.DictionarySize;
					this.dummy[dummyIndex++] = (0 <= pos ? pos : -1);
				}
			}
			this.primaryCount[i] = 0;
		}
		final int[] temp = this.secondaryHashTable;
		this.secondaryHashTable = this.dummy;
		this.dummy = temp;
		//------------------------------------------------------------------
		//  連結リストを更新
		for (int i = 0; i < this.prev.length; i++) {
			final int pos = this.prev[i] - this.DictionarySize;
	...
	private void divide(final int dbase, final int sbase, final int divoff) {
		final int limit = this.DictionarySize;
		int position = this.secondaryHashTable[sbase];
		final int[] current = { -1, -1, -1, -1 };
		//------------------------------------------------------------------
		//  連結リストを分岐させていくループ
		while (limit < position) {
			final int shash = this.TextBuffer[position + divoff] & 0x03;
			if (0 < current[shash]) {
