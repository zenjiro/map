...
public class LhaRetainedOutputStream extends OutputStream {
	 ...
	 * @see LhaProperty
	 */
	public LhaRetainedOutputStream(final String filename, final Properties property)
	...
	public void putNextEntryAlreadyCompressed(final LhaHeader header) throws IOException {
		if (header.getOriginalSize() != LhaHeader.UNKNOWN
			...
			this.archive.write(header.getBytes(this.encoding)); //throws IOException
	...
	public void putNextEntryNotYetCompressed(final LhaHeader header) throws IOException {
		if (header.getOriginalSize() != LhaHeader.UNKNOWN) {
			...
			this.out = CompressMethod.connectEncoder(this.rafo, header.getCompressMethod(),
					this.property);
		} else {
			throw new IllegalArgumentException("OriginalSize must not \"LhaHeader.UNKNOWN\".");
		}
	...
	public void closeEntry() throws IOException {
		if (this.header != null) {
			...
			if (!this.rafo.cache.isEmpty()) {
				...
				in = CompressMethod.connectDecoder(rafi, this.header.getCompressMethod(),
						this.property, this.header.getOriginalSize());
			...
			final long pos = this.archive.getFilePointer();
			final long size = (pos - this.headerpos - this.header.getBytes(this.encoding).length);
	...
	private static class RandomAccessFileOutputStream extends OutputStream {
		 ...
		 */
		public RandomAccessFileOutputStream(final RandomAccessFile archive, final long length)
				throws IOException {
	...
	private static class RandomAccessFileInputStream extends InputStream {
		 ...
		 */
		public RandomAccessFileInputStream(final RandomAccessFile archive,
				final RandomAccessFileOutputStream out) {
			this.archive = archive;
			this.pos = out.start;
			this.limit = out.pos;
		...
		public void cache(final long pos) throws IOException {
			final int length = (int) Math.min(this.limit - this.pos, pos - this.pos);
	...
	private static class Cache {
		...
		public int read() {
			if (null != this.current) {
				final int ret = this.current[this.position++] & 0xFF;
		...
		public int read(final byte[] buffer, final int index, final int length) {
			...
			while (null != this.current && count < length) {
				final int copylen = Math.min(this.current.length - this.position, length - count);
