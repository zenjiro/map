...
public class LhaFile {
	 ...
	 * @see LhaProperty
	 */
	public LhaFile(final RandomAccessFile file, final Properties property, final boolean rescueMode)
			throws IOException {
		this.constructerHelper(file, property, rescueMode);
	}
	 ...
	 *                 encodeがサポートされない場合
	 */
	private void constructerHelper(final RandomAccessFile file, final Properties property, boolean rescueMode)
			throws IOException {
		this.headers = new Vector<LhaHeader>();
		this.entryPoint = new Vector<Long>();
		file.seek(0);
		...
		this.duplicate = new Vector<Integer>();
		for (int i = 0; i < this.headers.size(); i++) {
			final LhaHeader header = this.headers.elementAt(i);
	...
	public InputStream getInputStream(final LhaHeader header) {
		final int index = this.getIndex(header);
		if (0 <= index) {
			final long start = this.entryPoint.elementAt(index).longValue();
			final long len = header.getCompressedSize();
			final InputStream in = new RandomAccessFileInputStream(start, len);
			return CompressMethod.connectDecoder(in, header.getCompressMethod(), this.property,
	...
	public InputStream getInputStream(final String name) {
		if (this.hash.containsKey(name)) {
			final int index = this.hash.get(name).intValue();
			final LhaHeader header = this.headers.elementAt(index);
			final long start = this.entryPoint.elementAt(index).longValue();
			final long len = header.getCompressedSize();
			final InputStream in = new RandomAccessFileInputStream(start, len);
			return CompressMethod.connectDecoder(in, header.getCompressMethod(), this.property,
	...
	private static boolean equal(final LhaHeader header1, final LhaHeader header2) {
		return header1.getPath().equals(header2.getPath())
				...
				&& header1.getCRC() == header2.getCRC() && header1.getOSID() == header2.getOSID()
	...
	private class RandomAccessFileInputStream extends InputStream {
		...
		public int read() throws IOException {
			synchronized (LhaFile.this.archive) {
				if (this.position < this.end) {
					...
					final int data = LhaFile.this.archive.read();
					if (0 <= data)
						this.position++;
					return data;
				} else {
		...
		public long skip(final long length) {
			synchronized (LhaFile.this.archive) {
				final long skiplen = Math.min(this.end - this.position, length);
	...
	private static class CachedRandomAccessFileInputStream extends InputStream {
		...
		public int read(final byte[] buffer, int index, int length) throws IOException {
			...
			while (0 < length) {
				...
				final int copylen = Math.min(length, this.cacheLimit - this.cachePosition);
		...
		public long skip(long length) throws IOException {
			...
			if (this.cachePosition < this.cacheLimit) {
				final long avail = (long) this.cacheLimit - this.cachePosition;
				final long skiplen = Math.min(length, avail);
				length -= skiplen;
				this.cachePosition += (int) skiplen;
			}
			if (0 < length) {
				final long avail = this.archive.length() - this.archive.getFilePointer();
		...
		private void fillCache() throws IOException {
			...
			while (0 <= read && this.cacheLimit < this.cache.length) {
				read = this.archive.read(this.cache, this.cacheLimit, this.cache.length
						- this.cacheLimit);//throws IOException
				if (0 < read)
