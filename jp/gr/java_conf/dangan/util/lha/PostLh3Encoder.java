...
public class PostLh3Encoder implements PostLzssEncoder {
	...
	public PostLh3Encoder(final OutputStream out, final int BufferSize) {
		...
		if (out != null && MinCapacity <= BufferSize) {
			...
			throw new IllegalArgumentException(
					"BufferSize too small. BufferSize must be larger than " + MinCapacity);
		}
	}
	...
	private void writeOut() throws IOException {
		...
		if (0 < this.blockSize) {
			...
			//------------------------------------------------------------------
			//  ハフマン符号表生成
			final int[] codeLen = StaticHuffman.FreqListToLenList(this.codeFreq);
			final int[] codeCode = StaticHuffman.LenListToCodeList(codeLen);
			final int[] offHiLen = PostLh3Encoder.getBetterOffHiLen(this.offHiFreq, StaticHuffman
					.FreqListToLenList(this.offHiFreq));
			...
			for (int i = 0; i < this.blockSize; i++) {
				...
				if (0 == (this.buffer[this.flagPos] & this.flagBit)) {
					final int code = this.buffer[this.position++] & 0xFF;
					this.out.writeBits(codeLen[code], codeCode[code]); //throws IOException
				} else {
					final int code = (this.buffer[this.position++] & 0xFF) | 0x100;
					final int offset = ((this.buffer[this.position++] & 0xFF) << 8)
