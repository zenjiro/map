...
public class LhaInputStream extends InputStream {
	 ...
	 *               encode がサポートされない場合
	 */
	private void constructerHelper(final InputStream in, final Properties property)
	...
	public int read() throws IOException {
		if (this.in != null) {
			final int ret = this.in.read(); //throws IOException
			if (ret < 0) {
	...
	public int read(final byte[] buffer, final int index, final int length) throws IOException {
		if (this.in != null) {
			final int ret = this.in.read(buffer, index, length); //throws IOException
			if (ret < 0) {
				this.reachedEndOfEntry = true;
			}
			return ret;
	...
	public long skip(final long length) throws IOException {
		if (this.in != null) {
			if (0 < length) {
				final long len = this.in.skip(length - 1); //throws IOException
				final int ret = this.in.read(); //throws IOException
				if (ret < 0) {
					this.reachedEndOfEntry = true;
					return len;
	...
	public LhaHeader getNextEntry() throws IOException {
		if (!this.reachedEndOfArchive) {
			...
			if (null != HeaderData) {
				final LhaHeader header = LhaHeader.createInstance(HeaderData, this.property);
				this.in = new DisconnectableInputStream(this.source);
				this.limit = new LimitedInputStream(this.in, header.getCompressedSize());
				this.in = CompressMethod.connectDecoder(this.limit, header.getCompressMethod(),
						this.property, header.getOriginalSize());
				this.reachedEndOfEntry = false;
