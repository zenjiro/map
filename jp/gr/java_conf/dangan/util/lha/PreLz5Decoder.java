...
public class PreLz5Decoder implements PreLzssDecoder {
	...
	public int readCode() throws IOException {
		...
		if (0 != (this.flagByte & this.flagBit)) {
			...
			this.flagBit <<= 1;
			final int c1 = this.in.read(); //throws IOException
			final int c2 = this.in.read(); //throws IOException
			if (0 <= c1) {
	...
	public int readOffset() throws IOException {
		final int offset = (this.position - this.matchPos - 1 - PreLz5Decoder.MaxMatch)
	...
	public int available() throws IOException {
		return Math.max(this.in.available() * 8 / 9 - 2, 0); //throws IOException
	}
