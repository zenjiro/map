...
public class PreLh5Decoder implements PreLzssDecoder {
		...
		if (CompressMethod.LH4.equals(method) || CompressMethod.LH5.equals(method)
			...
			if (in != null && 0 < CodeTableBits && 0 < OffLenTableBits) {
				...
				throw new IllegalArgumentException(
						"CodeTableBits too small. CodeTableBits must be larger than 1.");
			} else {
				throw new IllegalArgumentException(
						"OffHiTableBits too small. OffHiTableBits must be larger than 1.");
			}
		} else if (null == method) {
	...
	public int readCode() throws IOException {
		...
		if (this.bitCount < 16) {
			if (2 <= this.cacheLimit - this.cachePosition) {
				...
				final int len = this.codeLen[~node];
	...
	public int readOffset() throws IOException {
		...
		int offlen = ~node;
		final int len = this.offLenLen[offlen];
		this.bitBuffer <<= len;
		this.bitCount -= len;
		offlen--;
		if (0 <= offlen) {
	...
	public int available() throws IOException {
		int avail = ((this.cacheLimit - this.cachePosition) + this.in.available()
				/ this.cache.length * this.cache.length);//throws IOException
	...
	private void readBlockHead() throws IOException {
		...
		if (null != this.codeLen) {
			final short[][] tableAndTree = StaticHuffman.createTableAndTree(this.codeLen,
					this.codeTableBits);//throws BadHuffmanTableException
		...
		if (null != this.offLenLen) {
			final short[][] tableAndTree = StaticHuffman.createTableAndTree(this.offLenLen,
					this.offLenTableBits);//throws BadHuffmanTableException
	...
	private int[] readCodeLenLen() throws IOException {
		final int listlen = this.readBits(5); //throws BitDataBrokenException, EOFException, IOException
		if (0 < listlen) {
	 ...
	 * @param codeLenTable 長さ表
	 * @param codeLenLen 長さの長さ
	 * 
	 * @return ハフマン符号長のリスト。
	 *         符号長のリストが無い場合は null
	 * 
	...
	private int[] readCodeLen(final short[] codeLenTable, final int[] codeLenLen) throws IOException {
		...
		final int listlen = this.readBits(9); //throws BitDataBrokenException, EOFException, IOException
		if (0 < listlen) {
			final int[] codeLen = new int[listlen];
			int index = 0;
			while (index < listlen) {
				this.fillBitBuffer();
				final int bits = (0 < codeLenTableBits ? this.bitBuffer >>> (32 - codeLenTableBits) : 0);
	...
	private int[] readOffLenLen() throws IOException {
		final int listlen = this.readBits(Bits.len(Bits.len(this.DictionarySize)));//throws BitDataBrokenException, EOFException, IOException
		if (0 < listlen) {
	...
	private boolean readBoolean() throws IOException {
		if (0 < this.bitCount) {
			final boolean bool = (this.bitBuffer < 0);
			this.bitBuffer <<= 1;
			this.bitCount -= 1;
			return bool;
		} else {
	...
	private int readBits(int count) throws IOException {
		if (0 < count) {
			if (count <= this.bitCount) {
				final int bits = this.bitBuffer >>> (32 - count);
