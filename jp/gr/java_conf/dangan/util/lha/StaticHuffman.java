...
public class StaticHuffman {
	...
	public static int[] FreqListToLenList(final int[] FreqList) {
		...
		if (2 <= LeafCount) {
			...
			final int[] LenFreq = StaticHuffman.HuffmanTreeToLenFreq(SmallNode, LargeNode, TreeCount - 1);
	...
	public static int[] LenListToCodeList(final int[] LenList) throws BadHuffmanTableException {
		//ハフマン符号長の頻度表
		final int[] LenFreq = new int[StaticHuffman.LimitLen + 1];
		//ハフマン符号長に対応した符号
		final int[] CodeStart = new int[StaticHuffman.LimitLen + 2];
		//ハフマン符号長の頻度表作成
	...
	public static short[] createTable(final int[] LenList) throws BadHuffmanTableException {
		final int[] CodeList = StaticHuffman.LenListToCodeList(LenList); //throws BadHuffmanTableException
		int TableBits = 0;
		int LastCode = 0;
		for (int i = 0; i < LenList.length; i++) {
		...
		for (int i = 0; i < LenList.length; i++) {
			if (0 < LenList[i]) {
				...
				final int end = (i != LastCode ? start + (1 << (TableBits - LenList[i])) : Table.length);
	 ...
	 *                  ハフマン符号表が生成出来ない場合
	 */
	public static short[][] createTableAndTree(final int[] LenList, final int TableBits)
		...
		for (int i = 0; i < LenList.length; i++) {
			if (0 < LenList[i]) {
				...
				if (TreeBits <= 0) {
					final int start = CodeList[i] << (TableBits - LenList[i]);
					final int end = (i != LastCode ? start + (1 << (TableBits - LenList[i]))
							: Table.length);
					for (int j = start; j < end; j++) {
						Table[j] = (short) ~i;
					}
	...
	private static void MergeSort(final int[] array, final int first, final int last, final int[] weight, final int[] work) {
		if (first < last) {
			...
			while (srcIndex <= last && workIndex < middle - first)
				array[dstIndex++] = (weight[work[workIndex]] < weight[array[srcIndex]] ? work[workIndex++]
						: array[srcIndex++]);
	 ...
	 */
	private static void internalHuffmanTreeToLenFreq(final int[] SmallNode, final int[] LargeNode, final int node,
			final int len, final int[] LenFreq) {
		if (node < (SmallNode.length + 1) / 2) {
			//nodeが葉なら頻度表更新
			LenFreq[(len < StaticHuffman.LimitLen ? len : StaticHuffman.LimitLen)]++;
		} else {
			//nodeがノードなら両方のノードを再帰的に探索
			StaticHuffman.internalHuffmanTreeToLenFreq(SmallNode, LargeNode, SmallNode[node],
					len + 1, LenFreq);
			StaticHuffman.internalHuffmanTreeToLenFreq(SmallNode, LargeNode, LargeNode[node],
					len + 1, LenFreq);
		}
	}
}
