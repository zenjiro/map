...
public class LzssOutputStream extends OutputStream {
	 ...
	 * @param LzssSearchMethodClassName 
	 *                LzssSearchMethod の実装を示すパッケージ名も含めたクラス名
	 * @param LzssSearchMethodExtraArguments 引数
	 * 
	 * @exception NoClassDefFoundError
	 *              LzssSearchMethodClassName で与えられたクラスが
	 *              見つからない場合。
	 ...
	 */
	public LzssOutputStream(final PostLzssEncoder encoder, final String LzssSearchMethodClassName,
			final Object[] LzssSearchMethodExtraArguments) {
		...
		try {
			this.method = (LzssSearchMethod) Factory.createInstance(LzssSearchMethodClassName,
	...
	public void write(final byte[] buffer, final int index, final int length) throws IOException {
		...
		while (pos < end) {
			final int space = this.TextBuffer.length - this.writtenPos;
			if (end - pos < space) {
	...
	private void encode(boolean last) throws IOException {
		int end = Math.min(this.TextBuffer.length - this.MaxMatch, this.writtenPos
				- (last ? 0 : this.method.putRequires()));
		if (this.searchPos < end) {
			...
			while (true) {
				...
				final int lastmatchoff = this.searchPos - matchpos - 1;
	...
	private void slide() {
		...
		this.method.slide();
		if (this.lastsearchret != LzssOutputStream.NEEDSEARCH) {
			final int matchlen = LzssOutputStream.getMatchLen(this.lastsearchret);
			final int matchpos = LzssOutputStream.getMatchPos(this.lastsearchret);
			this.lastsearchret = LzssOutputStream.createSearchReturn(matchlen, matchpos
					- this.DictionarySize);
		}
		this.writtenPos -= this.DictionarySize;
		this.searchPos -= this.DictionarySize;
