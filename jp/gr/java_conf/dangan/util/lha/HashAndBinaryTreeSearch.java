...
public class HashAndBinaryTreeSearch implements LzssSearchMethod {
	 ...
	 */
	public HashAndBinaryTreeSearch(final int DictionarySize, final int MaxMatch, final int Threshold,
	...
	public void put(final int position) {
		...
		final int max = position + this.MaxMatch;
		int p = 0;
		int s = 0;
		while (scanpos != HashAndBinaryTreeSearch.UNUSED) {
			...
			parentpos = scanpos;
			scanpos = (buf[s] < buf[p] ? this.large[scanpos & (this.DictionarySize - 1)]
		...
		if (this.hashTable[hash] != HashAndBinaryTreeSearch.UNUSED) {
			...
			final int node = position & (this.DictionarySize - 1);
	...
	public int search(final int position, final int lastPutPos) {
		...
		final byte[] buf = this.TextBuffer;
		final int max = Math.min(this.TextBuffer.length, position + this.MaxMatch);
		int s = 0;
		int p = 0;
		int len = 0;
	...
	private void addNode(final int parentpos, final int position, final int len) {
		final int parentnode = parentpos & (this.DictionarySize - 1);
		final int node = position & (this.DictionarySize - 1);
	...
	private void deleteNode(final int position) {
		...
		if (this.parent[node] != HashAndBinaryTreeSearch.UNUSED) {
			if (this.small[node] == HashAndBinaryTreeSearch.UNUSED
				...
				final int replace = this.findNext(position);
	...
	private void contractNode(final int oldpos, final int newpos) {
		final int oldnode = oldpos & (this.DictionarySize - 1);
		final int newnode = newpos & (this.DictionarySize - 1);
		final int parentpos = this.parent[oldnode];
		final int parentnode = parentpos & (this.DictionarySize - 1);
