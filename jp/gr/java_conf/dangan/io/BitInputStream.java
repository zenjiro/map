...
public class BitInputStream extends InputStream {
	...
	public void mark(int readLimit) {
		...
		readLimit += 4;
		readLimit = ((readLimit / this.cache.length) * this.cache.length + (readLimit
				% this.cache.length == 0 ? 0 : this.cache.length));
	...
	public int readBit() throws IOException {
		if (0 < this.bitCount) {
			final int bit = this.bitBuffer >>> 31;
			this.bitBuffer <<= 1;
			this.bitCount -= 1;
			return bit;
		} else {
			...
			if (count <= this.bitCount) {
				...
				if (count <= this.bitCount) {
					...
					if (count <= 32) {
						...
						count -= this.bitCount;
						count -= 32;
						final int pos = this.cachePosition + (count >> 3);
						count &= 0x07;
						if (0 < count) {
